...
PHASE_RE = re.compile(
    r"Phase37-ManifestElf:\s+discovered=(\d+),\s+executed=(\d+),\s+rejected=(\d+),\s+ok=(true|false)"
)
def cleanup():
    if os.name == "nt":
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            disc, execd, _, flag = m.groups()
            return int(disc) >= 3 and int(execd) >= 1 and flag == "true"
    return False
def main():
